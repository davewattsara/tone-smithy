...
- **Stage:** v1.0.0 shipped (tag `v1.0.0` on `main`). All v1 milestones (M0–M15) done. v1.1 in progress: **M16 (Quick wins, tag `m16`), M17 (Engine expansion, tag `m17`), M18 (Step sequencer, tag `m18`), and M19 (Cross-platform installers, tag `m19`) complete; M20 (factory bank expansion to ~120 presets + v1.1.0 release) is next** — M19 landed the "filter 2 optional (off/serial/parallel, default off)" engine pre-req and the OSC2/OSC3 pan mod destinations, so M20 authors against the full v1.1 engine. M20 plan in `docs/planning/06-implementation/m20-plan.md`; awaiting explicit go-ahead before implementation. v1.0 ships unsigned with the default icon (both deferred to a later version).
