...
- **Stage:** **v1.2.0 shipped (tag `v1.2.0` on `main`, 2026-06-20).** All v1 (M0–M15), v1.1 (M16–M20), and v1.2 (M21–M24) milestones are complete. v1.2 milestones: M21 (UX polish, `m21`), M22 (engine additions, `m22`), M23 (oscillator phase consistency, `m23`), M24 (auto-update check + the v1.2.0 release, `v1.2.0`). The factory bank is 120 presets. Builds ship unsigned with the default icon (both deferred to a later version). **No milestone is currently active** — the next band is v1.3 (Expression & tuning); await explicit go-ahead before starting any new work.
- **Licence:** `MIT OR Apache-2.0` dual.
- **Product name:** Tone Smithy.
- **Branches:** `main` (stable; v1.2.0 at tag `v1.2.0`); `development` (active work — commit here).
...
- **v1.2 scope (shipped):** M21 UX polish + M22 engine additions (second sequencer mod lane `Seq2`; editable "Custom" FM operator routing) + M23 oscillator phase consistency (per-oscillator Free/Retrig phase mode; single-main-oscillator init default with a factory-preset migration via `cargo xtask migrate-osc-defaults`) + M24 best-effort GitHub-Releases update check & the v1.2.0 release. Milestones M21–M24 in `docs/planning/06-implementation/milestones.md`.
- **Release:** v1.0.0, v1.1.0, and v1.2.0 are live on GitHub Releases. Pushing a `v*` tag triggers the release workflow (Windows + Linux + macOS artefacts). NB: the sandbox can't push (no GitHub token) — the user pushes tags from their host; the repo working dir is a host mount, so sandbox commits/tags are already on disk for them.
- **Open questions remaining:** MPE/microtuning/oversampling scope (v1.3+). Auto-update mechanism is **resolved** (shipped in v1.2.0). Factory content authoring is resolved as solo (developer + Claude); code signing and the custom icon are resolved as "deferred to a later version" (see `docs/planning/01-vision/open-questions.md`).
