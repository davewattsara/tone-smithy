...
- [x] Open the FM panel. Both slots show **Subtractive** selected. Slot 0
  level knob reads ~1.00; slot 1 level reads 0.00.
- [x] Hold a note — you hear the standard saw. Release.
- [x] On **Slot 1**, click **FM**. Hold a note — still hear only slot 0
  saw (slot 1 level is 0.00). No crash.
- [x] Raise **Slot 1 level** to ~0.50. Hold a note — you hear a mix of
...
- [x] Click **Subtractive** again on slot 1. Hold a note — the FM
  contribution disappears; only the saw remains. Release.
- [x] Switch **Slot 0** to **FM** and raise its level if needed.
...
- [x] Hold a note. Move the **Slot 1 Pan** knob left — FM sound pans to
  the left channel. Right — pans right. C — balanced. Release.
- [x] Move **Slot 0 Pan** right. Hold a note. Saw comes from the right,
  FM from the left (or wherever you set each). A hybrid stereo image.
  Return both pans to C. Release.
- [x] Set slot 1 level to 0.00 — FM goes silent mid-note. Set back to 0.50.
...
- [x] **Algorithm 1** (Op 4→Op 3→Op 2→Op 1 stack, Op 1 is the carrier):
...
- [x] **Algorithm 2** (same stack + Op 4 self-feedback):
...
- [x] **Algorithm 3** (two stacks: Op 4→Op 3 and Op 2→Op 1, mixed):
...
- [x] **Algorithm 4** (Op 4 modulates Op 1, Op 2, Op 3 in parallel):
...
- [x] **Algorithm 5** (Op 4 modulates Op 2+Op 3; Op 3 modulates Op 1):
...
- [x] **Algorithm 6** (Op 3+Op 2 modulate Op 1; Op 4 separate carrier):
...
- [x] **Algorithm 7** (all four parallel, additive):
...
- [x] **Algorithm 8** (Op 4→Op 1; Op 3→Op 2; Op 1 and Op 2 are carriers):
...
- [x] Set **Op 4 Ratio Fine** to +100 ct. Hold a note — the timbre
...
- [x] Set **Op 4 Level** (top modulator in the stack) to 0.00. Hold a note.
...
- [x] Set Op 4 Level back to 1.00. Set **Op 3 Level** to 0.00. Now Op 3
  contributes no modulation to Op 2, so the remaining chain is just
  Op 2 → Op 1 — effectively a simple FM pair.
- [x] Set all levels to 0.00 except **Op 1 Level** = 1.00. Algorithm 1 only
...
- [x] Set **Op 4 Attack** to 2.00 s. Hold a note. The timbre should start
...
- [x] Set **Op 4 Decay** to 100 ms, **Op 4 Sustain** = 0.00. Hold a note.
...
- [x] Set **Op 1 Attack** to 1.00 s (the carrier's own envelope). Hold a
...
- [x] Hold a note. You should hear a warm saw (slot 0, slightly left)
  blended with a bell-like FM tone (slot 1, slightly right). Both
  layers should be clearly audible and stereo-imaged.
- [x] Play a scale across 2+ octaves. Both layers track pitch correctly.
- [x] Play several notes in quick succession (short staccato). Both
  layers start and stop cleanly with no hanging notes.
- [x] While holding a note, move **Slot 0 Pan** to L100 (full left) and
  **Slot 1 Pan** to R100 (full right). The saw and FM bell should be
  clearly separated in the stereo field.
- [x] Set **Slot 1 Level** to 0.00. Only the saw remains. Set to 1.00. FM
...
- [x] Hold a high note (C5 or above). The output should sound dense and
...
- [x] Play the same patch at C2 (low) then C5 (high). Timbre should get
...
- [x] Set sustain ON. Rapidly play 32 different notes (use the virtual
  keyboard to tap across its full range). All 32 should sound. Check
  the footer **voice count** reads 32.
- [x] Watch the **CPU %** in the footer. With 32 voices and FM active
  on slot 1, it should stay well below 50%. If it exceeds 50%, note
  the value here: `______%`.
- [x] Release sustain. All voices fade and the count drops to 0.
- [x] No audio dropout, no click, no crash during the above.
...
- [x] Enable mod matrix Slot 1: Source = LFO1, Dest = **Vol** (master
...
- [x] Change to Source = Env2, Dest = **Cutoff**, Amount = 5000. Hold a
...
- [x] With **both slots set to Subtractive**, the synth sounds identical
  to pre-M7 (warm saw, filter, mod matrix all work as before).
- [x] Filter Cutoff and Resonance knobs respond. LFO and Env2 panels
  show live readouts.
- [x] Virtual keyboard, computer keyboard, pitch bend, mod wheel, and
  sustain all work normally.
- [x] Scrolling the main window still reaches the virtual keyboard at
  the bottom.
- [x] Footer CPU% and voice count update in real time.
- [x] No audio dropout or crash during normal play after several minutes.
